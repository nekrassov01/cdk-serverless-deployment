...
import { Common, IEnvironmentConfig, IFunctionConfig, IResourceConfig } from "./common";
export interface AppStackProps extends StackProps {
  domainName: string;
  environmentConfig: IEnvironmentConfig;
  resourceConfig: IResourceConfig;
  functions: IFunctionConfig[];
}
...
export class AppStack extends Stack {
  constructor(scope: Construct, id: string, props: AppStackProps) {
    ...
    const { environmentConfig, resourceConfig, domainName, functions } = props;
    const common = new Common();
    const apiDefinitionPath = "src/apigateway/definition/openapi.yaml";
    ...
    const functionConfigs = [
      {
        ...
        functionName: "item1",
        description: "item1",
        codePath: `backend/item1/${resourceConfig.lambda.package}`,
      },
      {
        id: "Item2Function",
        functionName: "item2",
        description: "item2",
        codePath: `backend/item2/${resourceConfig.lambda.package}`,
      },
      {
        id: "Item1FunctionV2",
        functionName: "item1-v2",
        description: "item1-v2",
        codePath: `backend-v2/item1/${resourceConfig.lambda.package}`,
      },
      {
        id: "Item2FunctionV2",
        functionName: "item2-v2",
        description: "item2-v2",
        codePath: `backend-v2/item2/${resourceConfig.lambda.package}`,
      },
    ];

    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    let apiDefinition: any = yaml.load(readFileSync(apiDefinitionPath, "utf8"));

    const functionData = functionConfigs
      .map((config) => {
        common.createLambdaFunction(this, config.id, {
          functionName: config.functionName,
          description: config.description,
          runtime: lambda.Runtime.NODEJS_18_X,
          handler: "index.handler",
          code: lambda.Code.fromBucket(functionBucket, config.codePath),
          parameterStore: false,
        });

        const arn = `arn:aws:lambda:${this.region}:${this.account}:function:${common.getResourceName(
          config.functionName
        )}:${common.resourceConfig.lambda.alias}`;
        const functionMapping = functions.find((func) => func.name === config.functionName);

        return functionMapping ? { functionMapping, arn } : null;
      })
      .filter(Boolean) as { functionMapping: IFunctionConfig; arn: string }[];

    apiDefinition = common.updateApiDefinition(apiDefinition, functionData);
    ...
    const api = new apig.SpecRestApi(this, "RestApi", {
      apiDefinition: apig.ApiDefinition.fromInline(apiDefinition),
    ...
    const distribution = new cloudfront.Distribution(this, "Distribution", {
      ...
      additionalBehaviors: {
        [`/${api.deploymentStage.stageName}/*`]: {
          origin: new cloudfront_origins.HttpOrigin(`${api.restApiId}.execute-api.${this.region}.${this.urlSuffix}`, {
